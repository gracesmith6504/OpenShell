...
but requires the privileged OpenShell CNI DaemonSet to install pod-network
rules during CNI `ADD` using nftables or iptables instead of a privileged pod
init container. Proxy-pod mode moves network enforcement into a paired
...
in the alternate topologies. They preserve gateway session and SSH behavior,
but treat the process leaf as network-only: Landlock filesystem policy,
