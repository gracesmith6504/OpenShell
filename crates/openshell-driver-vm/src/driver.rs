...
impl VmDriver {
    ...
    async fn build_cached_local_image_rootfs_image(
        ...
        let sandbox_group_id = self.config.resolve_sandbox_gid(sandbox_user_id);
        self.publish_vm_progress(
            sandbox_id,
            "PreparingRootfs",
            format!(
                "Preparing VM rootfs for local image \"{image_ref}\" (sandbox uid={sandbox_user_id})"
            ),
            HashMap::from([
                ("image_ref".to_string(), image_ref.to_string()),
                ("image_source".to_string(), "local_docker".to_string()),
                ("image_identity".to_string(), image_identity.to_string()),
                ("sandbox_uid".to_string(), sandbox_user_id.to_string()),
            ]),
        );
    ...
    async fn build_cached_registry_image_rootfs_image(
        ...
        self.publish_vm_progress(
            ...
            "PreparingRootfs",
            format!(
                "Preparing VM rootfs for image \"{image_ref}\" (sandbox uid={sandbox_user_id})"
            ),
            HashMap::from([
                ("image_ref".to_string(), image_ref.to_string()),
                ("image_source".to_string(), "registry".to_string()),
