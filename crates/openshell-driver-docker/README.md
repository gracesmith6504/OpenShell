...
| CDI GPU request | Uses explicit `template.driver_config.gpu_device_ids` when set and its unique entry count equals `resource_requirements.gpu.count`; otherwise requests all NVIDIA GPUs when `resource_requirements.gpu` is present and daemon CDI support is detected. Count-only GPU requests are rejected until Docker CDI selection can map counts to concrete devices. |
