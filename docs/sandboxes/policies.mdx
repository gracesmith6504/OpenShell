...
containment prover does not model fail closed in managed mode. Unmanaged gateways keep their
existing behavior. An attached provider must have a resolvable policy profile so the gateway can
include its credential-bearing reach in the proof.
