...
keywords: "Generative AI, Cybersecurity, Gateway, Docker, Podman, Kubernetes, MicroVM, Slurm, CLI"
position: 2
---
...
| Slurm | Apptainer-backed jobs on Slurm compute nodes. | HPC clusters where the gateway runs on a login node. |
...
<Tip>
For driver setup, including Docker, Podman, MicroVM, Kubernetes, and Slurm paths, refer to [Installation](/about/installation).
...
| Slurm | Gateway logs, Slurm job state, generated job logs under the Slurm work directory, and Apptainer image availability. |
