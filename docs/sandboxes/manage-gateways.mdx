...
Use `openshell info` when you need elevated runtime details such as initialized
compute drivers and driver-reported capability versions:
...
```shell
openshell info -o json
```

Use `openshell gateway info` when you need local registration details such as
the registered endpoint, gateway metadata source, or authentication mode:
