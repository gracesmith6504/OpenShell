...
Request a specific driver-native device with `--gpu-device`:
...
but do not support count-based selection yet. The CLI sets the portable GPU
count to match the requested device ID. VM gateways accept only one GPU. In the
API, portable GPU presence and count populate
`SandboxSpec.resource_requirements.gpu`. Exact device selection is passed as
driver-owned `template.driver_config.gpu_device_ids`. Drivers that support
exact selection require the number of unique `gpu_device_ids` entries to match
`resource_requirements.gpu.count`.
