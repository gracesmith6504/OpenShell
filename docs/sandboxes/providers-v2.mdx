...
When a gateway enables one credential driver, the create/update API stores
submitted provider credentials in that backend and persists only internal
credential handles. When no credential driver is enabled, OpenShell stores
provider credentials in the gateway database.
