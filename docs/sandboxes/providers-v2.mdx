...
| Multiple managed policy inputs | Managed admission composes attached provider policy into each candidate and checks it against one gateway-managed maximum. Selecting and intersecting multiple maximums for agents and subagents is future work. |
