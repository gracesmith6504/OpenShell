...
### Managed Permission Modes

When the gateway has a [managed maximum policy](/sandboxes/policies#managed-maximum-policies),
`--permission-mode ask|auto` selects the sandbox's immutable managed mode. Omit the flag to use the
maximum's default. This mode replaces `proposal_approval_mode` for that sandbox:

- `ask` keeps every in-boundary authority expansion pending.
- `auto` immediately applies only expansions inside the maximum's auto-eligible region.
- Authority marked `review.required` stays pending in either mode.
- Outside, explicitly denied, or unsupported proposals are returned in `rejection_reasons` and do
  not enter the draft queue.

OpenShell reruns containment against the live sandbox policy and provider state immediately before
an auto or human-approved proposal is merged. The stored proposal and earlier decision explain what
happened; they do not authorize a stale candidate.
...
5. The gateway runs admission against the proposed delta. Unmanaged sandboxes store accepted proposals as draft chunks and use the security-finding prover below. Managed sandboxes first reject proposals outside their configured maximum.
