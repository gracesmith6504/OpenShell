...
# Optional provider credential storage backend. When omitted or empty,
# provider credentials remain stored in the gateway database.
...
Set `credential_drivers` to the credential driver the gateway uses to store provider credentials. OpenShell supports at most one enabled credential driver at a time. If `credential_drivers` is omitted or empty, provider credentials remain stored in the gateway database.
