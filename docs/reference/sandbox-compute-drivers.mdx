...
| `supervisor_topology` | `supervisor.topology` | Set `combined` for the default single supervisor path, `sidecar` to move pod-level network enforcement and gateway session handling into a dedicated sidecar, `cni-sidecar` to use that sidecar runtime model with CNI-installed pod-network rules, or `proxy-pod` to run network enforcement and gateway forwarding in a separate supervisor Deployment with NetworkPolicy isolation. |
...
policy still runs in the sidecar and remains endpoint/L7 scoped in sidecar and
cni-sidecar topologies.

CNI-sidecar mode keeps the sidecar runtime model and local snapshot boundary,
but requires the privileged OpenShell CNI DaemonSet to install nftables or
iptables pod-network rules before the workload starts.
