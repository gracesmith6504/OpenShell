...
| `supervisor_topology` | `supervisor.topology` | Set `combined` for the default single supervisor path, `sidecar` to move pod-level network enforcement and gateway forwarding into a dedicated sidecar, or `split-pod` to run network enforcement in a separate supervisor Deployment with NetworkPolicy isolation. |
...
and SSH behavior, but the process supervisor runs in network-only mode:
filesystem policy, process privilege dropping, and
