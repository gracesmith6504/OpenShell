...
| Slurm | Experimental for Apptainer-backed HPC sandboxes. | Requires a Slurm login node, compute-node callback reachability, Apptainer, and shared storage for driver state. |
...
| Slurm | Site dependent | Required for Slurm-backed gateways. The gateway host must provide `sbatch`, `srun`, `squeue`, and `scancel`. |
| Apptainer | Site dependent | Required by the Slurm driver MVP to run OCI/SIF sandbox images on compute nodes. |
