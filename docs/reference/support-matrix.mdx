...
| Network namespaces, veth, and nftables | Required    | Linux 3.13 or later | Network policy enforcement cannot start.                                       |
| Landlock LSM                           | Recommended | Linux 5.13 or later | `best_effort` runs degraded with a security finding; `hard_requirement` fails. |
| cgroups v2 PID controller              | Recommended | Linux 4.5 or later  | The compute driver or container runtime must provide PID limits.               |
