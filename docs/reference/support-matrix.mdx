...
| Network namespaces, veth, and nftables | Required    | 3.13    | Network policy enforcement cannot start.                                       |
| Landlock LSM                           | Recommended | 5.13    | `best_effort` runs degraded with a security finding; `hard_requirement` fails. |
| cgroups v2 PID controller              | Recommended | 4.5     | The compute driver or container runtime must provide PID limits.               |
| User namespaces                        | Optional    | 5.12    | Stronger Kubernetes host isolation is unavailable.                             |
