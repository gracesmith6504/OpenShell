...
keywords: "Generative AI, Cybersecurity, AI Agents, Sandboxing, Installation, Setup, Gateway, Docker, Podman, MicroVM, Kubernetes, Slurm"
position: 3
---
...
| Slurm | The gateway runs on a Slurm login node and submits Apptainer-backed sandbox jobs with `sbatch` and `srun`. | Slurm client commands, Apptainer, a shared filesystem visible to login and compute nodes, and a compute-node-reachable gateway endpoint. |
...
Slurm deployments run the gateway on a login node and configure `OPENSHELL_DRIVERS=slurm`. The Slurm driver requires `OPENSHELL_GRPC_ENDPOINT` to point at a gateway URL reachable from compute nodes and `OPENSHELL_SLURM_SUPERVISOR_BIN` to point at the Linux `openshell-sandbox` binary on a shared filesystem.

For local validation and demos, run:
