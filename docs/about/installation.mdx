...
```shell
# Other targets: aarch64-unknown-linux-gnu, aarch64-apple-darwin
mkdir -p "$HOME/.local/bin"
curl -LsSf "https://github.com/NVIDIA/OpenShell/releases/latest/download/openshell-gateway-x86_64-unknown-linux-gnu.tar.gz" \
  | tar -xz -C "$HOME/.local/bin" openshell-gateway
chmod 755 "$HOME/.local/bin/openshell-gateway"
export PATH="$HOME/.local/bin:$PATH"
```

To configure the gateway process, refer to [Configuring the Gateway](/sandboxes/manage-gateways#configuring-the-gateway).
