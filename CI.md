...
- `test:e2e-centos` runs the CentOS Stream 10 Docker/Podman/SELinux
  compatibility suite in `Branch E2E Checks`. It boots a CentOS Stream 10
  VM under QEMU/KVM on the runner, installs Docker alongside Podman, and
  runs both `e2e:docker` and `e2e:podman` against a real SELinux-enforcing
  kernel. Reproduce it locally with `mise run vm:centos-selinux:create`
  (see `tasks/scripts/vm/centos-selinux-vm.sh`).
...
| `.github/workflows/branch-e2e.yml` | Opt-in standard, GPU, Kubernetes HA, and CentOS Stream 10 SELinux E2E. Triggers on `push: pull-request/[0-9]+` and runs jobs selected by `test:e2e`, `test:e2e-gpu`, `test:e2e-kubernetes`, or `test:e2e-centos`. |
